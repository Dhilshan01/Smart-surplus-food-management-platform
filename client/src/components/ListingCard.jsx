...
const ListingCard = ({ listing, showActions, onClaim, onDelete }) => {
  const safety =
    safetyConfig[listing.live_safety?.score || listing.safety_score] ||
    safetyConfig.safe;
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
      ...
      <div
        className={`h-1.5 w-full ${
          listing.live_safety?.score === "safe"
            ? "bg-green-500"
            : listing.live_safety?.score === "moderate_risk"
              ? "bg-yellow-400"
              : "bg-red-500"
        }`}
      />
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-2">
          ...
          <span
            className={`ml-2 shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${safety.bg} ${safety.text} ${safety.border}`}
          >
        ...
        {listing.donor_name && (
          <div className="text-xs text-gray-400 mb-4 border-t pt-3">
            ...
            <span className="font-medium text-gray-600">
              {listing.organization_name || listing.donor_name}
            </span>
          </div>
        )}
